$(document).ready(function(){
  ...
  function appendUsers(users_array){
    ...
    //console.log("Users array received", users_array);
    for(var i = 0; i < users_array.length; i++){
      var post = users_array[i]; //we are storing the posts in a temp variable for easy access, we don't have to write users_array[i] everytime
  ...
  function attachUsersEvents(){
    console.log("Attaching events on the links");
    console.log("I found " + $("a").length + " links on the page. Attaching events only on those.") //Now this will find one link available on the page and the count wil show 1
    $(".user a").on('click', function(event){ // any link inside a class .post
      ...
      $.ajax({
        ...
        success: function(response){
          //Now we have the data with us for user with id user_id
